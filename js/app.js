...
function exportGameHtml(){const g=loadGames().find(x=>x.id===state.selectedGameId); if(!g)return; download(`registro-leitura-jogo-${g.date||'sem-data'}-${pdfFileSafe(g.opponent||'adversario')}.html`, gameHtmlReport(g));}
function exportPeriodHtml(){download(`metricas-${$('#periodStart').value||'inicio'}-${$('#periodEnd').value||'fim'}.html`, periodHtmlReport());}
