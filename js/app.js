...
function exportGameHtml(){const g=loadGames().find(x=>x.id===state.selectedGameId); if(!g)return; download(`jogo-${g.date||'sem-data'}-${pdfFileSafe(g.opponent||'adversario')}.html`, gameHtmlReport(g), 'text/html;charset=utf-8');}
function exportPeriodHtml(){download(`metricas-${$('#periodStart').value||'inicio'}-${$('#periodEnd').value||'fim'}.html`, periodHtmlReport(), 'text/html;charset=utf-8');}
...
/* === AveliCoach 2.2.3 ajustes de layout/exportação === */
function gameEvolutionSets(game){
  const sc=scoreTimeline(game); const bySet={};
  sc.gameEvents.forEach(ev=>{ (bySet[ev.set] ||= []).push({score:`${ev.games.athlete}–${ev.games.opponent}`, winner:ev.winner, time:ev.time, index:ev.index}); });
  return Object.keys(bySet).sort((a,b)=>+a-+b).map(k=>({set:k, chips:bySet[k]}));
}
function gameEvolutionStripHtml(game){
  const sets=gameEvolutionSets(game);
  if(!sets.length) return '<div class="game-evolution-strip"><strong>Evolução dos games</strong><span class="muted">Ainda sem games fechados.</span></div>';
  return `<div class="game-evolution-strip"><strong>Evolução dos games</strong>${sets.map(s=>`<div class="game-evo-row"><span>Set ${s.set}</span><div class="game-evo-chips">${s.chips.map(c=>`<i class="${c.winner==='athlete'?'won-athlete':'won-opponent'}" title="${escapeHtml(c.time||'fim de game')}">${escapeHtml(c.score)}</i>`).join('')}</div></div>`).join('')}<small><b></b> jogador venceu o game · <b class="opp"></b> adversário venceu o game</small></div>`;
}
function renderPresenceChart(target,game){
  const data=presenceData(game); if(!data.length){target.innerHTML='<p class="muted">Sem pontos suficientes para mostrar presença no tempo.</p>';return;}
  const W=900,H=278,m={l:52,r:18,t:24,b:40},pw=W-m.l-m.r,ph=H-m.t-m.b;
  const x=i=>m.l+(data.length===1?pw/2:i*pw/(data.length-1)); const y=v=>m.t+ph-((v+100)/200)*ph;
  const path=key=>data.map((d,i)=>(i?'L':'M')+x(i).toFixed(1)+' '+y(d[key]).toFixed(1)).join(' ');
  const ticks=[-100,-50,0,50,100],step=Math.max(1,Math.ceil(data.length/6));
  const evs=scoreTimeline(game).gameEvents; const vlines=evs.map(ev=>{const xx=x(Math.min(ev.index,data.length-1)).toFixed(1);return `<line class="presence-game-line" x1="${xx}" x2="${xx}" y1="${m.t}" y2="${H-m.b}"></line>`}).join('');
  target.innerHTML=`<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Presença no jogo">${ticks.map(t=>`<line class="${t===0?'presence-line-zero':'presence-grid'}" x1="${m.l}" x2="${W-m.r}" y1="${y(t)}" y2="${y(t)}"></line><text class="presence-axis-label" x="${m.l-8}" y="${y(t)+4}" text-anchor="end">${t}</text>`).join('')}${vlines}<path class="presence-player" d="${path('athlete')}"></path><path class="presence-opponent" d="${path('opponent')}"></path>${data.map((d,i)=>`<circle class="presence-dot-player" cx="${x(i)}" cy="${y(d.athlete)}" r="2.2"></circle><circle class="presence-dot-opponent" cx="${x(i)}" cy="${y(d.opponent)}" r="2.2"></circle>`).join('')}${data.map((d,i)=>i%step===0||i===data.length-1?`<text class="presence-axis-label" x="${x(i)}" y="${H-13}" text-anchor="middle">${escapeHtml(d.label)}</text>`:'').join('')}<circle class="presence-dot-player" cx="${m.l}" cy="14" r="5"></circle><text class="presence-axis-label" x="${m.l+10}" y="18">Jogador</text><circle class="presence-dot-opponent" cx="${m.l+82}" cy="14" r="5"></circle><text class="presence-axis-label" x="${m.l+92}" y="18">Adversário</text><line class="presence-game-line" x1="${W-180}" x2="${W-162}" y1="14" y2="14"></line><text class="presence-axis-label" x="${W-156}" y="18">fim de game</text></svg>${gameEvolutionStripHtml(game)}`;
}
function pdfTheme(){return{ink:[22,32,38],muted:[96,120,122],accent:[0,132,96],line:[169,185,185],panel:[243,248,246],panel2:[238,245,243],cyan:[67,191,211],green:[81,224,164],blue:[85,184,255],yellow:[255,200,90]};}
function pdocCard(doc,x,y,w,h){const t=pdfTheme();doc.setDrawColor(...t.line);doc.setFillColor(...t.panel);doc.roundedRect(x,y,w,h,3,3,'FD')}
function oneBar(doc,x,y,w,h,title,obj,den){
  const t=pdfTheme(); pdocCard(doc,x,y,w,h); pdocText(doc,title.toUpperCase(),x+3,y+7,5.5,t.accent,true);
  const entries=Object.entries(obj||{}).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]).slice(0,5),max=Math.max(1,...entries.map(([,v])=>v)); let yy=y+16;
  if(!entries.length){pdocText(doc,'Sem dados.',x+3,yy,5.2,t.muted);return}
  const total=den||entries.reduce((s,[,vv])=>s+vv,0)||1; const rowGap=Math.min(8, Math.max(6.3,(h-19)/entries.length));
  entries.forEach(([k,v])=>{const bw=w-18; pdocText(doc,pdfLabel(k),x+3,yy,4.7,t.ink); doc.setFillColor(222,234,232); doc.roundedRect(x+3,yy+2.1,bw,2.1,1,1,'F'); doc.setFillColor(...t.cyan); doc.roundedRect(x+3,yy+2.1,Math.max(1,bw*v/max),2.1,1,1,'F'); pdocText(doc,`${v} (${pct(v,total)}%)`,x+w-3,yy+3.4,4.5,t.ink,true,{align:'right'}); yy+=rowGap;});
}
function drawScoreBySetPdf(doc,x,y,w,h,tl){const t=pdfTheme();pdocCard(doc,x,y,w,h);pdocText(doc,'PLACAR POR SET',x+3,y+7,5.8,t.accent,true);const sets=tl.setScores.length?tl.setScores:[{set:1,athlete:0,opponent:0,current:true}];const cell=(w-8)/Math.max(1,sets.length);sets.slice(0,4).forEach((s,i)=>{const xx=x+4+i*cell;pdocText(doc,`SET ${s.set}`,xx+cell/2,y+21,4.8,t.muted,true,{align:'center'});pdocText(doc,`${s.athlete}–${s.opponent}${s.current?'*':''}`,xx+cell/2,y+35,11.5,t.ink,true,{align:'center'});});pdocText(doc,`Games totais: ${tl.totalGames.athlete}–${tl.totalGames.opponent} · ${tl.scoredPoints.athlete}–${tl.scoredPoints.opponent} pontos`,x+4,y+h-5,4.7,t.muted)}
function onePresence222(doc,x,y,w,h,game){
  const t=pdfTheme(),data=presenceData(game); pdocCard(doc,x,y,w,h); pdocText(doc,'PRESENÇA NO JOGO',x+3,y+7,5.8,t.accent,true); pdocText(doc,'● Jogador',x+32,y+17,4.6,t.green,true); pdocText(doc,'● Adversário',x+58,y+17,4.6,t.blue,true);
  if(!data.length){pdocText(doc,'Sem pontos suficientes.',x+4,y+26,5,t.muted);return}
  const px=x+9,py=y+20,pw=w-16,ph=h-35,n=data.length,xx=i=>px+(n===1?pw/2:i*pw/(n-1)),yy=v=>py+ph-((v+100)/200)*ph;
  [-100,-50,0,50,100].forEach(v=>{pdocLine(doc,px,yy(v),px+pw,yy(v),v===0?t.muted:[219,231,228],v===0?.4:.25); if(v%100===0) pdocText(doc,String(v),px-3,yy(v)+1.3,3.7,t.muted,false,{align:'right'});});
  scoreTimeline(game).gameEvents.forEach(ev=>{const gx=xx(Math.min(ev.index,n-1));pdocLine(doc,gx,py,gx,py+ph,t.yellow,.28);});
  const draw=(key,col)=>{doc.setDrawColor(...col);doc.setLineWidth(.55);data.forEach((d,i)=>{if(i===0)return;doc.line(xx(i-1),yy(data[i-1][key]),xx(i),yy(d[key]));}); doc.setFillColor(...col); data.forEach((d,i)=>{if(i%4===0||i===data.length-1)doc.circle(xx(i),yy(d[key]),.45,'F')});};
  draw('athlete',t.green); draw('opponent',t.blue);
  if(data.length){[0,Math.floor((data.length-1)/2),data.length-1].forEach(i=>pdocText(doc,data[i].label,xx(i),y+h-18,3.5,t.muted,false,{align:'center'}));}
  const sets=gameEvolutionSets(game); let cy=y+h-10; pdocText(doc,'Evolução dos games:',x+3,cy,4.3,t.accent,true); cy+=4.4;
  sets.slice(0,2).forEach(s=>{let cx=x+3; pdocText(doc,`Set ${s.set}`,cx,cy,3.8,t.muted,true); cx+=13; s.chips.slice(0,12).forEach(c=>{const col=c.winner==='athlete'?t.yellow:t.blue; doc.setFillColor(...col); doc.setDrawColor(...col); doc.roundedRect(cx,cy-3.2,9.2,4.2,1.2,1.2,'FD'); pdocText(doc,c.score,cx+4.6,cy-.2,3.1,[255,255,255],true,{align:'center'}); cx+=10.2;}); cy+=4.8;});
}
function tableMetricsPdf222(doc,x,y,w,h,a){
  const t=pdfTheme(); pdocCard(doc,x,y,w,h); pdocText(doc,'COMPARATIVO TÉCNICO',x+3,y+7,5.8,t.accent,true);
  const rows=comparisonRows(a); const startY=y+17.5, rowH=(h-21)/rows.length;
  doc.setFillColor(232,242,239); doc.rect(x+2,y+10,w-4,6,'F');
  pdocText(doc,'Parâmetro',x+3,y+14.4,4.5,t.muted,true); pdocText(doc,'Jogador',x+w-72,y+14.4,4.5,t.muted,true,{align:'center'}); pdocText(doc,'Adversário',x+w-40,y+14.4,4.5,t.muted,true,{align:'center'}); pdocText(doc,'Ref.',x+w-11,y+14.4,4.5,t.muted,true,{align:'center'});
  rows.forEach((r,i)=>{const yy=startY+i*rowH;if(i%2===0){doc.setFillColor(248,251,250);doc.rect(x+2,yy-3.3,w-4,rowH,'F');}pdocText(doc,r.label,x+3,yy,3.95,t.ink,false);pdocText(doc,r.left,x+w-72,yy,4.05,t.ink,true,{align:'center'});pdocText(doc,r.mid,x+w-40,yy,4.05,t.ink,true,{align:'center'});pdocText(doc,r.ref,x+w-11,yy,4,t.ink,true,{align:'center'});});
}
function exportGamePdf(){
...
function presenceSvgReport222(game){
  ...
  const x=i=>m.l+(data.length===1?pw/2:i*pw/(data.length-1)), y=v=>m.t+ph-((v+100)/200)*ph, path=k=>data.map((d,i)=>(i?'L':'M')+x(i).toFixed(1)+' '+y(d[k]).toFixed(1)).join(' ');
  const vlines=scoreTimeline(game).gameEvents.map(ev=>{const xx=x(Math.min(ev.index,data.length-1)).toFixed(1); return `<line x1="${xx}" x2="${xx}" y1="${m.t}" y2="${m.t+ph}" stroke="#ffc85a" stroke-width="1" stroke-dasharray="4 4" opacity=".45"/>`;}).join('');
  return `<svg class="presence-report-svg" viewBox="0 0 ${W} ${H}">${[-100,-50,0,50,100].map(t=>`<line x1="${m.l}" x2="${W-m.r}" y1="${y(t)}" y2="${y(t)}" stroke="${t===0?'#91aaa9':'#dbe7e4'}"/><text x="${m.l-7}" y="${y(t)+4}" text-anchor="end" class="axis">${t}</text>`).join('')}${vlines}<path d="${path('athlete')}" class="presence-athlete"/><path d="${path('opponent')}" class="presence-opp"/>${data.map((d,i)=>i%4===0||i===data.length-1?`<circle cx="${x(i)}" cy="${y(d.athlete)}" r="1.8" class="dot-a"/><circle cx="${x(i)}" cy="${y(d.opponent)}" r="1.8" class="dot-o"/>`:'').join('')}<circle cx="${m.l}" cy="14" r="5" class="dot-a"/><text x="${m.l+10}" y="18" class="legend">Jogador</text><circle cx="${m.l+76}" cy="14" r="5" class="dot-o"/><text x="${m.l+86}" y="18" class="legend">Adversário</text></svg>${gameEvolutionStripHtml(game)}`;
}
function makeReportHtml222(title,subtitle,body){return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>:root{--ink:#142023;--muted:#637879;--green:#51e0a4;--blue:#55b8ff;--yellow:#ffc85a;--line:#31515a;--panel:#f3f8f6;--cyan:#43bfd3}*{box-sizing:border-box}body{margin:0;background:#f8fbfa;color:var(--ink);font-family:Inter,Segoe UI,Arial,sans-serif}.page{width:1120px;min-height:790px;margin:0 auto;padding:20px 22px}header{border-bottom:2px solid #a8b7b6;padding-bottom:10px;margin-bottom:10px}h1{font-size:27px;margin:0}header p{margin:8px 0 0;font-weight:800}.topgrid{display:grid;grid-template-columns:250px 1fr;gap:12px}.contentgrid{display:grid;grid-template-columns:610px 1fr;gap:12px;margin-top:12px}.sidecards{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.report-card{border:1.5px solid #9fb3b1;background:#f3f8f6;border-radius:12px;padding:13px;min-width:0;box-shadow:0 1px 0 #dfe9e7}h2{margin:0 0 13px;font-size:14px;color:#078264}.setgrid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;text-align:center;margin-top:22px}.setgrid span{display:block;color:#7b8e95;font-size:11px;font-weight:800}.setgrid b{display:block;margin-top:16px;font-size:25px}.muted{color:var(--muted);font-size:11px}.presence-report-svg{width:100%;height:210px}.axis,.legend{fill:#60777a;font:11px Arial}.presence-athlete{fill:none;stroke:#51e0a4;stroke-width:2.6}.presence-opp{fill:none;stroke:#55b8ff;stroke-width:2.6}.dot-a{fill:#51e0a4}.dot-o{fill:#55b8ff}.game-evolution-strip{margin-top:4px;border:1px solid #d5e1df;background:#eef5f3;border-radius:9px;padding:7px 9px}.game-evolution-strip strong{display:block;color:#078264;font-size:11px;margin-bottom:5px}.game-evo-row{display:flex;align-items:center;gap:8px;margin:4px 0}.game-evo-row>span{width:38px;font-weight:800;color:#637879;font-size:10px}.game-evo-chips{display:flex;flex-wrap:wrap;gap:4px}.game-evo-chips i{font-style:normal;color:#fff;font-size:10px;font-weight:800;border-radius:999px;padding:2px 6px}.won-athlete{background:#ffc85a}.won-opponent{background:#55b8ff}.game-evolution-strip small{display:block;margin-top:4px;color:#637879;font-size:9px}.game-evolution-strip small b{display:inline-block;width:9px;height:9px;border-radius:99px;background:#ffc85a}.game-evolution-strip small b.opp{background:#55b8ff}table{width:100%;border-collapse:collapse;font-size:10.4px}td,th{border-bottom:1px solid #d7e1df;padding:4.8px 4px;text-align:left}td:nth-child(n+2),th:nth-child(n+2){text-align:center}.report-bar{display:grid;grid-template-columns:72px 1fr 42px;gap:5px;align-items:center;font-size:9.2px;margin:4.5px 0}.report-track{height:7px;background:#dbe7e4;border-radius:99px;overflow:hidden}.report-track i{display:block;height:100%;background:linear-gradient(90deg,#51e0a4,#55b8ff)}.report-card.metrics{grid-row:span 2}.report-card p{line-height:1.35}@media print{body{background:#f8fbfa}.page{width:297mm;min-height:210mm;padding:14mm}}</style></head><body><main class="page"><header><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p></header>${body}</main></body></html>`;}
function exportGameHtml(){const g=loadGames().find(x=>x.id===state.selectedGameId); if(!g)return; download(`registro-leitura-jogo-${g.date||'sem-data'}-${pdfFileSafe(g.opponent||'adversario')}.html`, gameHtmlReport(g), 'text/html;charset=utf-8');}
function exportPeriodHtml(){download(`metricas-${$('#periodStart').value||'inicio'}-${$('#periodEnd').value||'fim'}.html`, periodHtmlReport(), 'text/html;charset=utf-8');}
function setup223(){const gp=$('#exportGamePdfBtn'); if(gp) gp.onclick=exportGamePdf; const gh=$('#exportGameHtmlBtn'); if(gh) gh.onclick=exportGameHtml; const pp=$('#exportPeriodPdfBtn'); if(pp) pp.onclick=exportPeriodPdf; const ph=$('#exportPeriodHtmlBtn'); if(ph) ph.onclick=exportPeriodHtml; if(state.selectedGameId){const g=loadGames().find(x=>x.id===state.selectedGameId); if(g) openGameDetail(g.id);}}
setup223();
...
/* =========================================================
   AveliCoach 2.2.4 - correção de overflow em cards PDF/HTML
   - cards de distribuição: Top 3 + Outros (máx. 4 linhas)
   - labels curtos no PDF
   - espaçamento e barras mais compactos
   - nomes de arquivos preservados sem “avelicoach”
   ========================================================= */
function pdfShortLabel(v){
  const k=String(v||'nao_informado');
  const map={
    nao_informado:'N/I', ponto_construido:'Construído', saque_direto:'Ace/serv.', dupla_falta:'Dupla falta',
    winner:'Winner', passada:'Passada', erro:'Erro', ace:'Ace',
    forehand:'Forehand', backhand:'Backhand', saque:'Saque', devolucao:'Devolução', voleio:'Voleio', smash:'Smash',
    rede:'Rede', fora_lado:'Fora lado', fora_fundo:'Fora fundo', corpo:'Corpo', fundo:'Fundo', cruzada:'Cruzada', paralela:'Paralela'
  };
  return pdfSafe(map[k] || labels[k] || k);
}
function compactEntries(obj,totalMax=4){
  const entries=Object.entries(obj||{}).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  if(entries.length<=totalMax) return entries;
  const keep=entries.slice(0,totalMax-1);
  const other=entries.slice(totalMax-1).reduce((s,[,v])=>s+v,0);
  if(other>0) keep.push(['outros',other]);
  return keep;
}
function pdocCard(doc,x,y,w,h){
  const t=pdfTheme();
...
function oneBar(doc,x,y,w,h,title,obj,den){
  ...
  pdocText(doc,title.toUpperCase(),x+3,y+6.2,5.35,t.accent,true);
  const entries=compactEntries(obj,4), max=Math.max(1,...entries.map(([,v])=>v));
  const total=den||entries.reduce((s,[,vv])=>s+vv,0)||1;
  let yy=y+15;
  const rowGap=5.75;
  const bw=w-24;
  if(!entries.length){pdocText(doc,'Sem dados.',x+3,yy,5.4,t.muted);return;}
  entries.forEach(([k,v])=>{
    if(yy > y+h-4) return;
    const label=k==='outros'?'Outros':pdfShortLabel(k);
    pdocText(doc,label,x+3,yy,4.45,t.ink,false);
    doc.setFillColor(225,237,234); doc.roundedRect(x+3,yy+1.5,bw,1.9,.9,.9,'F');
    doc.setFillColor(...t.cyan); doc.roundedRect(x+3,yy+1.5,Math.max(.8,bw*v/max),1.9,.9,.9,'F');
    pdocText(doc,`${v} (${pct(v,total)}%)`,x+w-3,yy+2.7,4.25,t.ink,true,{align:'right'});
    yy += rowGap;
  });
}
function barsHtml222(title,obj,total){
  const entries=compactEntries(obj,5);
  return `<div class="report-card"><h2>${escapeHtml(title)}</h2>${entries.length?entries.map(([k,v])=>`<div class="report-bar"><span>${escapeHtml(k==='outros'?'Outros':pdfShortLabel(k))}</span><div class="report-track"><i style="width:${pct(v,total)}%"></i></div><b>${v} (${pct(v,total)}%)</b></div>`).join(''):'<p class="muted">Sem dados suficientes.</p>'}</div>`;
}
function exportGamePdf(){
  const JS=pdfCheckLib(); if(!JS)return; const g=loadGames().find(x=>x.id===state.selectedGameId); if(!g)return; const a=analyze([g]),tl=scoreTimeline(g),doc=new JS({orientation:'landscape',unit:'mm',format:'a4'}),t=pdfTheme();
  doc.setFillColor(248,251,250); doc.rect(0,0,297,210,'F');
...
/* =========================================================
   AveliCoach 2.3.2 - índices estilo TV, nível e gráficos
   ========================================================= */
const INDEX_INFO_232 = {
  pontos_vencidos:{title:'Pontos vencidos',what:'Percentual de pontos vencidos em relação aos pontos disputados.',use:'Dá a visão mais direta do desempenho global da partida.',calc:'pontos vencidos ÷ total de pontos × 100'},
  saque:{title:'Pontos vencidos no saque',what:'Percentual de pontos ganhos quando o jogador estava sacando.',use:'Mede a eficiência global do jogo de serviço.',calc:'pontos vencidos sacando ÷ pontos sacando × 100'},
  games_saque_vencidos:{title:'Games de saque vencidos',what:'Percentual de games em que o jogador confirma seu próprio saque.',use:'Mostra a capacidade de transformar eficiência de saque em manutenção efetiva do serviço.',calc:'games de saque vencidos ÷ games de saque disputados × 100'},
  devolucao:{title:'Pontos vencidos na devolução',what:'Percentual de pontos ganhos quando o jogador estava devolvendo.',use:'Mede a capacidade de pressionar o saque adversário.',calc:'pontos vencidos devolvendo ÷ pontos devolvendo × 100'},
  pontos_erro:{title:'Erros',what:'Percentual de pontos perdidos por erro do jogador.',use:'Mostra o custo dos próprios erros na partida.',calc:'erros ÷ total de pontos × 100'},
  dupla_falta:{title:'Duplas faltas',what:'Frequência de pontos perdidos diretamente por dupla falta.',use:'Mostra o custo direto da instabilidade do saque.',calc:'duplas faltas ÷ pontos de saque × 100'},
  aces:{title:'Aces (saques diretos)',what:'Frequência de pontos ganhos diretamente pelo saque.',use:'Mede a produção direta de pontos pelo saque.',calc:'aces ÷ pontos de saque × 100'},
  pressao:{title:'Pontos vencidos sob pressão',what:'Percentual de pontos vencidos nas situações classificadas como de maior pressão.',use:'Avalia o rendimento competitivo em momentos críticos.',calc:'pontos sob pressão vencidos ÷ pontos sob pressão × 100'},
  pontos_positivos:{title:'Pontos positivos',what:'Pontos ganhos por winner (bola vencedora), passada, ace (saque direto) ou ponto construído.',use:'Mostra quanto o jogador produz ativamente para vencer pontos.',calc:'pontos positivos ÷ total considerado × 100'},
  primeiro_saque_ganho:{title:'Pontos ganhos no 1º saque',what:'Percentual de pontos vencidos quando o primeiro saque iniciou efetivamente o ponto.',use:'Separa a eficácia específica do primeiro saque do desempenho geral no saque.',calc:'pontos vencidos após 1º saque ÷ pontos iniciados com 1º saque × 100'},
  break_points_salvos:{title:'Break points salvos',what:'Percentual de oportunidades de quebra enfrentadas que o jogador consegue salvar.',use:'Mede a capacidade de proteger o saque em situações críticas.',calc:'break points salvos ÷ break points enfrentados × 100'},
  break_points_convertidos:{title:'Break points convertidos',what:'Percentual de oportunidades de quebra no saque adversário que o jogador converte.',use:'Mede a capacidade de transformar pressão na devolução em quebra de saque.',calc:'break points convertidos ÷ break points disponíveis × 100'},
  erros_forcados:{title:'Erros forçados',what:'Parcela dos erros cometidos em situação defensiva ou em corrida, conforme a regra atual do AveliCoach.',use:'Diferencia erros provocados pela pressão adversária dos demais erros.',calc:'erros forçados ÷ total de erros × 100'},
  pontos_curtos_eficiencia:{title:'Eficiência em pontos curtos - até 4 golpes',what:'Percentual de vitória nos pontos que terminaram em até quatro golpes.',use:'Mede a eficácia competitiva na fase inicial do ponto.',calc:'pontos vencidos com até 4 golpes ÷ total de pontos com até 4 golpes × 100'},
  direcao_saque_eficiencia:{title:'Eficiência por direção do saque',what:'Percentual de pontos ganhos após saques no T, corpo e aberto.',use:'Mostra quais escolhas de direção produzem melhores resultados.',calc:'pontos ganhos em cada direção ÷ saques naquela direção × 100'},
  saque_mais_um_dentro:{title:'Eficiência do Saque +1 dentro da quadra',what:'Percentual de pontos vencidos quando a terceira bola foi executada dentro da quadra.',use:'Mede se a conquista de posição ofensiva após o saque está sendo convertida em resultado.',calc:'pontos ganhos com Saque +1 dentro ÷ ocorrências de Saque +1 dentro × 100'},
  primeiro_saque_dentro:{title:'1º saque dentro',what:'Percentual de tentativas de primeiro saque que entram.',use:'Mede estabilidade e consistência técnica do primeiro saque.',calc:'1º saques dentro ÷ tentativas de 1º saque × 100'},
  segundo_saque_ganho:{title:'Pontos ganhos no 2º saque',what:'Percentual de pontos vencidos quando foi necessário utilizar o segundo saque.',use:'Acompanha a evolução da segurança e da qualidade competitiva do segundo saque.',calc:'pontos vencidos após 2º saque ÷ pontos iniciados com 2º saque × 100'},
  pontos_curtos_share:{title:'Pontos curtos - até 4 golpes',what:'Percentual de pontos encerrados em até quatro golpes.',use:'Mostra como a estrutura do jogo está se organizando em torno das primeiras bolas.',calc:'pontos com até 4 golpes ÷ pontos com nº de golpes registrado × 100'},
  erros_nao_forcados:{title:'Erros não forçados',what:'Erros que não se enquadram como forçados pela regra atual do sistema.',use:'Acompanha estabilidade e controle da execução.',calc:'erros não forçados ÷ total de erros × 100'},
  construcao:{title:'Construção do ponto',what:'Percentual de pontos registrados como construídos.',use:'Mostra se o atleta está conseguindo desenvolver o ponto antes do desfecho.',calc:'pontos construídos ÷ total considerado × 100'},
  direcao_saque:{title:'Direção do saque',what:'Distribuição dos saques entre T, corpo e aberto.',use:'Acompanha repertório, variedade e organização tática do saque.',calc:'saques em cada direção ÷ total de saques registrados × 100'},
  posicao_saque_1:{title:'Posição do Saque +1',what:'Distribuição da terceira bola entre atrás da linha, na linha e dentro da quadra.',use:'Acompanha se o saque está permitindo ao atleta conquistar progressivamente posições mais ofensivas.',calc:'ocorrências em cada posição ÷ Saques +1 registrados × 100'},
  consistencia:{title:'Consistência de troca',what:'Índice de solidez do jogador durante as trocas.',use:'Acompanha a capacidade de sustentar rallies com estabilidade.',calc:'fórmula atual do índice de solidez do AveliCoach'},
  agressividade:{title:'Agressividade controlada',what:'Índice próprio que relaciona produção ofensiva e custo em erros.',use:'Acompanha se o atleta consegue aumentar a iniciativa ofensiva sem perder controle.',calc:'fórmula composta atual do AveliCoach'}
};
Object.entries(INDEX_INFO_232).forEach(([key,info])=>{infoTexts['idx_'+key]={title:info.title,html:raw`<p><strong>O que é:</strong> ${info.what}</p><p><strong>Para que serve:</strong> ${info.use}</p><p><strong>Como é calculado:</strong> ${info.calc}</p>`};});

const INDEX_SECTIONS_232 = [
  {title:'Índices de resultado', keys:['pontos_vencidos','saque','games_saque_vencidos','devolucao','pontos_erro','dupla_falta','aces','pressao','pontos_positivos','primeiro_saque_ganho','break_points_salvos','break_points_convertidos','erros_forcados','pontos_curtos_eficiencia','direcao_saque_eficiencia','saque_mais_um_dentro']},
...
function pctSafe(count,total){return (Number(total)||0)?pct(count,total):0;}
function rowMetric(count,total){const p=pctSafe(count,total);return {count:Number(count)||0,total:Number(total)||0,pct:p,display:`${count}/${Number(total)||0} (${p}%)`,short:`${p}%`};}
function pctMetric(p){p=Number(p)||0;return {count:p,total:100,pct:p,display:`${p}%`,short:`${p}%`};}
function getPointDate(p){return p.savedDate||p.matchDate||p.gameDate||'';}
function normalizeLevel(g){return (g?.level||g?.nivel||'classe') || 'classe';}
function levelLabel(g){return labels[normalizeLevel(g)] || 'Classe';}
function pointSide(p,side){return p.winner===side||p.actor===side||p.server===side;}
function isShortRally(p){return ['1','2','3','4'].includes(String(p.rallyLength||''));}
function hasRally(p){return ['1','2','3','4','5_8','9_plus'].includes(String(p.rallyLength||''));}
function serveAttemptPoints(a,side,attempt){return a.points.filter(p=>p.server===side && p.serveAttempt===attempt && ['athlete','opponent'].includes(p.winner));}
function serveDirectionPoints(a,side){return a.points.filter(p=>p.server===side && ['aberta','corpo','t'].includes(p.serveDirection) && ['athlete','opponent'].includes(p.winner));}
function servePlusInsidePoints(a,side){return a.points.filter(p=>p.server===side && p.servePlusPosition==='dentro_quadra' && ['athlete','opponent'].includes(p.winner));}
function gamesByServer(g){const out={athlete:{total:0,won:0},opponent:{total:0,won:0}}, pts=g.points||[];let gameStart=0;for(let i=0;i<pts.length;i++){const before=recalcScore(pts.slice(0,i));const after=recalcScore(pts.slice(0,i+1));const bg=before.totalGames.athlete+before.totalGames.opponent,ag=after.totalGames.athlete+after.totalGames.opponent;if(ag>bg){const gp=pts[gameStart]||pts[i];const server=gp.server||'athlete';const winner=after.totalGames.athlete>before.totalGames.athlete?'athlete':'opponent';out[server].total++;if(winner===server)out[server].won++;gameStart=i+1;}}return out;}
function gamesByServerFromGames(games){const out={athlete:{total:0,won:0},opponent:{total:0,won:0}};(games||[]).forEach(g=>{const r=gamesByServer(g);['athlete','opponent'].forEach(s=>{out[s].total+=r[s].total;out[s].won+=r[s].won;});});return out;}
function enrichAnalysis232(a){const gbs=gamesByServerFromGames(a.games||[]);const sp1A=serveAttemptPoints(a,'athlete','primeiro'),sp1O=serveAttemptPoints(a,'opponent','primeiro'),sp2A=serveAttemptPoints(a,'athlete','segundo'),sp2O=serveAttemptPoints(a,'opponent','segundo');const dirA=serveDirectionPoints(a,'athlete'),dirO=serveDirectionPoints(a,'opponent');const inA=servePlusInsidePoints(a,'athlete'),inO=servePlusInsidePoints(a,'opponent');const rallyPts=a.points.filter(hasRally),shortPts=a.points.filter(isShortRally);return {gbs,sp1A,sp1O,sp2A,sp2O,dirA,dirO,inA,inO,rallyPts,shortPts};}
Object.assign(comparisonMetricDefs, {
  games_saque_vencidos:{label:'Games de saque vencidos',info:'idx_games_saque_vencidos',benchmark:70,mode:'min',ref:'≥ 70%',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.gbs.athlete.won,e.gbs.athlete.total)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.gbs.opponent.won,e.gbs.opponent.total)}},
  primeiro_saque_ganho:{label:'Pontos ganhos no 1º saque',info:'idx_primeiro_saque_ganho',benchmark:65,mode:'min',ref:'≥ 65%',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.sp1A.filter(p=>p.winner==='athlete').length,e.sp1A.length)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.sp1O.filter(p=>p.winner==='opponent').length,e.sp1O.length)}},
  break_points_salvos:{label:'Break points salvos',info:'idx_break_points_salvos',benchmark:50,mode:'min',ref:'≥ 50%',player:a=>rowMetric(a.breakPoints?.athlete?.saved||0,a.breakPoints?.athlete?.against||0),opp:a=>rowMetric(a.breakPoints?.opponent?.saved||0,a.breakPoints?.opponent?.against||0)},
  break_points_convertidos:{label:'Break points convertidos',info:'idx_break_points_convertidos',benchmark:35,mode:'min',ref:'≥ 35%',player:a=>rowMetric(a.breakPoints?.athlete?.conv||0,a.breakPoints?.athlete?.opp||0),opp:a=>rowMetric(a.breakPoints?.opponent?.conv||0,a.breakPoints?.opponent?.opp||0)},
  pontos_curtos_eficiencia:{label:'Eficiência em pontos curtos - até 4 golpes',info:'idx_pontos_curtos_eficiencia',benchmark:55,mode:'min',ref:'≥ 55%',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.shortPts.filter(p=>p.winner==='athlete').length,e.shortPts.length)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.shortPts.filter(p=>p.winner==='opponent').length,e.shortPts.length)}},
  direcao_saque_eficiencia:{label:'Eficiência por direção do saque',info:'idx_direcao_saque_eficiencia',benchmark:55,mode:'min',ref:'≥ 55%',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.dirA.filter(p=>p.winner==='athlete').length,e.dirA.length)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.dirO.filter(p=>p.winner==='opponent').length,e.dirO.length)}},
  saque_mais_um_dentro:{label:'Eficiência do Saque +1 dentro da quadra',info:'idx_saque_mais_um_dentro',benchmark:60,mode:'min',ref:'≥ 60%',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.inA.filter(p=>p.winner==='athlete').length,e.inA.length)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.inO.filter(p=>p.winner==='opponent').length,e.inO.length)}},
  primeiro_saque_dentro:{label:'1º saque dentro',info:'idx_primeiro_saque_dentro',benchmark:58,mode:'min',ref:'≥ 58%',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.sp1A.length,e.sp1A.length+e.sp2A.length)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.sp1O.length,e.sp1O.length+e.sp2O.length)}},
  segundo_saque_ganho:{label:'Pontos ganhos no 2º saque',info:'idx_segundo_saque_ganho',benchmark:45,mode:'min',ref:'≥ 45%',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.sp2A.filter(p=>p.winner==='athlete').length,e.sp2A.length)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.sp2O.filter(p=>p.winner==='opponent').length,e.sp2O.length)}},
  pontos_curtos_share:{label:'Pontos curtos - até 4 golpes',info:'idx_pontos_curtos_share',benchmark:50,mode:'min',ref:'Ref.',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.shortPts.filter(p=>p.winner==='athlete').length,e.rallyPts.length)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.shortPts.filter(p=>p.winner==='opponent').length,e.rallyPts.length)}},
  direcao_saque:{label:'Direção do saque',info:'idx_direcao_saque',benchmark:75,mode:'min',ref:'Reg.',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.dirA.length,a.serveTotal)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.dirO.length,a.returnTotal)}},
  posicao_saque_1:{label:'Posição do Saque +1',info:'idx_posicao_saque_1',benchmark:50,mode:'min',ref:'Dentro',player:a=>{const e=enrichAnalysis232(a);return rowMetric(e.inA.length,a.serveTotal)},opp:a=>{const e=enrichAnalysis232(a);return rowMetric(e.inO.length,a.returnTotal)}}
});
['pontos_vencidos','saque','devolucao','pressao','pontos_positivos','pontos_erro','erros_forcados','erros_nao_forcados','agressividade','consistencia','construcao','dupla_falta','aces'].forEach(k=>{if(comparisonMetricDefs[k]){comparisonMetricDefs[k].mode=comparisonMetricDefs[k].ref&&comparisonMetricDefs[k].ref.includes('≤')?'max':'min'; comparisonMetricDefs[k].info='idx_'+k; if(INDEX_INFO_232[k]) comparisonMetricDefs[k].label=INDEX_INFO_232[k].title;}});
function comparisonRows(a){const rows=[];INDEX_SECTIONS_232.forEach(sec=>{rows.push({section:sec.title});sec.keys.forEach(key=>{const def=comparisonMetricDefs[key];if(!def)return;const left=def.player(a),mid=def.opp(a);rows.push({key,label:def.label,info:def.info||('idx_'+key),ref:def.ref||'',benchmark:Number(def.benchmark)||0,mode:def.mode||'min',left,leftDisplay:left.short||left.display,mid,midDisplay:mid.short||mid.display});});});return rows;}
function numericPctFromMetric(m){return Math.max(0,Math.min(100,Number(m?.pct)||0));}
function indexRowHtml232(r,withInfo=true){const at=numericPctFromMetric(r.left),ad=numericPctFromMetric(r.mid),bm=Math.max(0,Math.min(100,Number(r.benchmark)||0));return `<div class="index-tv-row ${state.gameTrendMetric===r.key?'active':''}" data-metric="${escapeHtml(r.key)}"><div class="index-name">${withInfo?`<button class="index-info" data-info="${escapeHtml(r.info)}" type="button">i</button>`:''}<span>${escapeHtml(r.label)}</span></div><div class="idx-val at">${escapeHtml(r.leftDisplay)}</div><div class="idx-bar-cell at"><span class="idx-bar-bg" style="width:${bm}%"></span><span class="idx-bar-real" style="width:${at}%"></span></div><div class="idx-axis"></div><div class="idx-bar-cell ad"><span class="idx-bar-bg" style="width:${bm}%"></span><span class="idx-bar-real" style="width:${ad}%"></span></div><div class="idx-val ad">${escapeHtml(r.midDisplay)}</div></div>`;}
function renderComparisonTable(target,a){const rows=comparisonRows(a);let html='<div class="index-tv"><div class="index-tv-head"><div>Índice</div><div class="at-head">AT</div><div class="ad-head">AD</div></div>';rows.forEach(r=>{if(r.section) html+=`<div class="index-section-title">${escapeHtml(r.section)}</div>`; else html+=indexRowHtml232(r,true);});html+='</div>';target.innerHTML=html;$$('.index-tv-row',target).forEach(row=>row.onclick=e=>{if(e.target.closest('.index-info'))return;state.gameTrendMetric=row.dataset.metric;const g=loadGames().find(x=>x.id===state.selectedGameId);if(g){renderComparisonTable(target,analyze([g]));renderGameInMatchTrend(g);renderGameParameterTrend(g);$('#gameInMatchTrendChart')?.scrollIntoView({behavior:'smooth',block:'center'});}});$$('.index-info',target).forEach(btn=>btn.onclick=e=>{e.stopPropagation();showInfo(btn.dataset.info);});}
function getGamesLastYearFor(targetGame){const end=new Date(targetGame.date||todayISO());const start=new Date(end);start.setFullYear(start.getFullYear()-1);const lvl=normalizeLevel(targetGame);return loadGames().filter(g=>{const d=new Date(g.date||todayISO());return d>=start&&d<=end&&normalizeLevel(g)===lvl;}).sort((a,b)=>(a.date||'').localeCompare(b.date||''));}
function renderGameParameterTrend(game){const def=comparisonMetricDefs[state.gameTrendMetric]||comparisonMetricDefs.pontos_vencidos;const games=getGamesLastYearFor(game);const playerSeries=games.map(g=>{const a=analyze([g]);const m=def.player(a);return{label:formatShortDate(g.date),value:numericPctFromMetric(m),display:m.display||m.short}});const oppSeries=games.map(g=>{const a=analyze([g]);const m=def.opp(a);return{label:formatShortDate(g.date),value:numericPctFromMetric(m),display:m.display||m.short}});const title=$('#gameParamTrendTitle'),sub=$('#gameParamTrendSubtitle');if(title)title.textContent=`Evolução entre jogos: ${def.label}`;if(sub){const lvl=levelLabel(game);sub.textContent=`Nível: ${lvl}${normalizeLevel(game)==='classe'?' — incluindo registros antigos sem nível':''}. Últimos 12 meses até ${game.date}. Azul claro = jogador. Dourado = adversário. Linha tracejada = referência COSAT U14.`;}renderLineChart($('#gameParamTrendChart'),{benchmark:def.benchmark||0,pointsA:playerSeries,pointsB:oppSeries,colorA:'#61d5ff',colorB:'#ffd166',nameA:'Jogador',nameB:'Adversário'});}
function pointValidForMetric(p,key){switch(key){case 'pontos_vencidos':return ['athlete','opponent'].includes(p.winner);case 'saque':return p.server==='athlete'&&['athlete','opponent'].includes(p.winner);case 'devolucao':return p.server==='opponent'&&['athlete','opponent'].includes(p.winner);case 'pressao':return isPressure(p)&&['athlete','opponent'].includes(p.winner);case 'pontos_positivos':return pointSide(p,'athlete');case 'pontos_erro':return pointSide(p,'athlete');case 'dupla_falta':return p.server==='athlete';case 'aces':return p.server==='athlete';case 'primeiro_saque_ganho':return p.server==='athlete'&&p.serveAttempt==='primeiro';case 'segundo_saque_ganho':return p.server==='athlete'&&p.serveAttempt==='segundo';case 'primeiro_saque_dentro':return p.server==='athlete'&&['primeiro','segundo'].includes(p.serveAttempt);case 'pontos_curtos_eficiencia':return hasRally(p)&&isShortRally(p);case 'pontos_curtos_share':return hasRally(p);case 'direcao_saque_eficiencia':return p.server==='athlete'&&['aberta','corpo','t'].includes(p.serveDirection);case 'direcao_saque':return p.server==='athlete';case 'saque_mais_um_dentro':return p.server==='athlete'&&p.servePlusPosition==='dentro_quadra';case 'posicao_saque_1':return p.server==='athlete';case 'erros_forcados':case 'erros_nao_forcados':return (p.ending==='erro'||p.ending==='dupla_falta')&&pointActor(p)==='athlete';case 'construcao':case 'consistencia':case 'agressividade':return pointSide(p,'athlete');case 'break_points_salvos':case 'break_points_convertidos':return false;default:return ['athlete','opponent'].includes(p.winner);}}
function pointSuccessForMetric(p,key,validSeen){switch(key){case 'pontos_vencidos':case 'saque':case 'devolucao':case 'pressao':case 'primeiro_saque_ganho':case 'segundo_saque_ganho':case 'pontos_curtos_eficiencia':case 'direcao_saque_eficiencia':case 'saque_mais_um_dentro':return p.winner==='athlete';case 'pontos_positivos':return p.actor==='athlete'&&['winner','passada','saque_direto','ponto_construido'].includes(p.ending);case 'pontos_erro':return !((p.ending==='erro'||p.ending==='dupla_falta')&&pointActor(p)==='athlete');case 'dupla_falta':return !(p.ending==='dupla_falta'&&pointActor(p)==='athlete');case 'aces':return p.ending==='saque_direto'&&p.actor==='athlete';case 'primeiro_saque_dentro':return p.serveAttempt==='primeiro';case 'pontos_curtos_share':return isShortRally(p);case 'direcao_saque':return ['aberta','corpo','t'].includes(p.serveDirection);case 'posicao_saque_1':return p.servePlusPosition==='dentro_quadra';case 'erros_forcados':return ['defesa','corrida'].includes(p.playType)||p.stroke==='corrida';case 'erros_nao_forcados':return !(['defesa','corrida'].includes(p.playType)||p.stroke==='corrida');case 'construcao':return p.actor==='athlete'&&(p.ending==='ponto_construido'||p.playType==='troca');case 'consistencia':return !((p.ending==='erro'||p.ending==='dupla_falta')&&pointActor(p)==='athlete');case 'agressividade':return p.winner==='athlete'||(p.actor==='athlete'&&['winner','passada','saque_direto','ponto_construido'].includes(p.ending));default:return p.winner==='athlete';}}
function inMatchSeriesForMetric(game,key){if(['break_points_salvos','break_points_convertidos'].includes(key))return inMatchBreakPointSeries(game,key);const pts=(game.points||[]).map((p,i)=>({...p,order:p.order||i+1,actor:pointActor(p),stroke:inferStrokeFromPoint(p)}));let valids=[],success=0;return pts.reduce((arr,p)=>{if(!pointValidForMetric(p,key))return arr;const ok=pointSuccessForMetric(p,key,valids.length);valids.push(ok);if(ok)success++;const cum=pct(success,valids.length);const win=valids.slice(-3);const mov=pct(win.filter(Boolean).length,win.length);arr.push({label:String(p.order),value:cum,display:`${success}/${valids.length} (${cum}%)`,moving:mov});return arr;},[]);}
function inMatchBreakPointSeries(game,key){const pts=game.points||[],sim=[];let valids=[],success=0,out=[];pts.forEach((p,i)=>{const pp={...p,order:p.order||i+1};const sc=recalcScore(sim),server=pp.server||'athlete',receiver=opposite(server);const isBp=canWinGameNext(sc,receiver);let valid=false,ok=false;if(key==='break_points_convertidos'&&isBp&&receiver==='athlete'){valid=true;ok=pp.winner==='athlete';}if(key==='break_points_salvos'&&isBp&&server==='athlete'){valid=true;ok=pp.winner==='athlete';}if(valid){valids.push(ok);if(ok)success++;const cum=pct(success,valids.length);const win=valids.slice(-3);out.push({label:String(pp.order),value:cum,display:`${success}/${valids.length} (${cum}%)`,moving:pct(win.filter(Boolean).length,win.length)});}sim.push(pp);});return out;}
function renderGameInMatchTrend(game){let wrap=$('#gameInMatchTrendBlock');if(!wrap){const hist=$('#gameParamTrendChart')?.closest('.detail-group');if(hist){wrap=document.createElement('div');wrap.id='gameInMatchTrendBlock';wrap.className='detail-group chart-card inmatch-trend-card';wrap.innerHTML='<div class="mini-head"><h3 id="gameInMatchTrendTitle">Evolução no jogo</h3><small id="gameInMatchTrendSubtitle">Acumulado e média móvel das últimas 3 ocorrências válidas.</small></div><div id="gameInMatchTrendChart" class="line-chart-wrap"></div>';hist.parentNode.insertBefore(wrap,hist);}}
 const def=comparisonMetricDefs[state.gameTrendMetric]||comparisonMetricDefs.pontos_vencidos;const series=inMatchSeriesForMetric(game,state.gameTrendMetric);$('#gameInMatchTrendTitle').textContent=`Evolução no jogo: ${def.label}`;$('#gameInMatchTrendSubtitle').textContent='Somente AT. Linha azul = acumulado. Linha dourada = média móvel das últimas 3 ocorrências válidas.';if(!series.length){$('#gameInMatchTrendChart').innerHTML='<p class="line-chart-empty">Dados insuficientes para esse índice durante o jogo.</p>';return;}renderLineChart($('#gameInMatchTrendChart'),{benchmark:def.benchmark||0,pointsA:series.map(s=>({label:s.label,value:s.value,display:s.display})),pointsB:series.map(s=>({label:s.label,value:s.moving,display:`${s.moving}%`})),colorA:'#61d5ff',colorB:'#ffd166',nameA:'Acumulado',nameB:'MM 3'});}
function renderDetailScore232(g,a,sc){const el=$('#gameScoreBanner');if(!el)return;el.innerHTML=`<div class="score-banner-pro"><div class="pro-score"><table class="score-pro"><thead><tr><th></th><th>S</th><th>G</th><th>P</th></tr></thead><tbody><tr><th>AT</th><td>${sc.sets.athlete}</td><td>${sc.totalGames.athlete}</td><td>${a.won}</td></tr><tr><th>AD</th><td>${sc.sets.opponent}</td><td>${sc.totalGames.opponent}</td><td>${a.lost}</td></tr></tbody></table></div></div>`;}
function openGameDetail(id){const g=loadGames().find(x=>x.id===id);if(!g)return;state.selectedGameId=id;state.gameTrendMetric=state.gameTrendMetric||'pontos_vencidos';const a=analyze([g]);const sc=recalcScore(g.points||[]);$('#detailTitle').textContent=`${g.date} • vs ${g.opponent||'Adversário'}`;$('#detailSubtitle').textContent=`${g.tournament||'Sem campeonato'} • ${levelLabel(g)} • ${g.startTime||''}${g.endTime?' às '+g.endTime:''}`;renderDetailScore232(g,a,sc);const simpleHead=$('#gameSimpleCards')?.previousElementSibling;if(simpleHead)simpleHead.classList.add('game-detail-hidden-section');$('#gameSimpleCards').innerHTML='';$('#gameSimpleText').textContent='';const matchGroup=$('#gameMatchCards')?.closest('.detail-group');if(matchGroup)matchGroup.classList.add('game-detail-hidden-section');renderComparisonTable($('#gameComparisonTable'),a);renderGameInMatchTrend(g);renderGameParameterTrend(g);renderBars($('#gameEndingChart'),countBy(a.athleteEndings,'ending'),a.athleteEndings.length);renderBars($('#gameStrokeChart'),countBy(a.athleteErrors,'stroke'),a.athleteErrors.length);renderBars($('#gamePlaceChart'),countBy(a.athleteErrors,'place'),a.athleteErrors.length);renderPresenceChart($('#gamePresenceChart'),g);bindComparisonInfo($('#gamePresenceChart').closest('.detail-group'));renderBars($('#gameOpponentEndingChart'),countBy(a.opponentEndings,'ending'),a.opponentEndings.length);renderBars($('#gameOpponentStrokeChart'),countBy(a.opponentErrors,'stroke'),a.opponentErrors.length);renderBars($('#gameOpponentPlaceChart'),countBy(a.opponentErrors,'place'),a.opponentErrors.length);renderCoachNotes($('#gameCoachNotes'),a,'player');renderCoachNotes($('#gameOpponentNotes'),a,'opponent');showScreen('gameDetail');}
...
function makeReportHtml222(title,subtitle,body){return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>:root{--ink:#142023;--muted:#637879;--line:#9fb3b1;--panel:#f3f8f6}*{box-sizing:border-box}body{margin:0;background:#f8fbfa;color:var(--ink);font-family:Inter,Segoe UI,Arial,sans-serif}.page{width:1120px;min-height:790px;margin:0 auto;padding:20px 22px}header{border-bottom:2px solid #a8b7b6;padding-bottom:10px;margin-bottom:10px}h1{font-size:27px;margin:0}header p{margin:8px 0 0;font-weight:800}.topgrid{display:grid;grid-template-columns:250px 1fr;gap:12px}.contentgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px}.report-card{border:1.5px solid #9fb3b1;background:#f3f8f6;border-radius:12px;padding:13px;min-width:0;box-shadow:0 1px 0 #dfe9e7}h2{margin:0 0 13px;font-size:14px;color:#078264}.muted{color:var(--muted);font-size:11px}.full{grid-column:1/-1}.report-bar{display:grid;grid-template-columns:72px 1fr 42px;gap:5px;align-items:center;font-size:9.2px;margin:4.5px 0}.report-track{height:7px;background:#dbe7e4;border-radius:99px;overflow:hidden}.report-track i{display:block;height:100%;background:#43bfd3}.index-tv{display:grid;gap:7px}.index-section-title{margin:7px 0 2px;color:#078264;font-weight:900;text-transform:uppercase;font-size:12px}.index-tv-head,.index-tv-row{display:grid;grid-template-columns:minmax(235px,1fr) 52px 130px 2px 130px 52px;gap:6px;align-items:center}.index-tv-head{font-size:11px;font-weight:900;color:#637879}.at-head{text-align:right;grid-column:2/4}.ad-head{text-align:left;grid-column:5/7}.index-tv-row{min-height:20px;border-bottom:1px solid #d6e2df}.index-name span{white-space:nowrap}.index-info{display:none}.idx-val{font-weight:900;font-variant-numeric:tabular-nums}.idx-val.at{text-align:right}.idx-val.ad{text-align:left}.idx-axis{height:18px;background:#142023;width:2px}.idx-bar-cell{position:relative;height:15px}.idx-bar-bg,.idx-bar-real{position:absolute;top:50%;transform:translateY(-50%);height:12px}.idx-bar-real{height:8px}.idx-bar-cell.at .idx-bar-bg,.idx-bar-cell.at .idx-bar-real{right:0}.idx-bar-cell.ad .idx-bar-bg,.idx-bar-cell.ad .idx-bar-real{left:0}.idx-bar-cell.at .idx-bar-bg{background:rgba(83,207,255,.18)}.idx-bar-cell.ad .idx-bar-bg{background:rgba(242,163,58,.18)}.idx-bar-cell.at .idx-bar-real{background:#53cfff}.idx-bar-cell.ad .idx-bar-real{background:#f2a33a}@media print{.page{width:297mm;min-height:210mm;padding:14mm}}</style></head><body><main class="page"><header><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p></header>${body}</main></body></html>`;}
function gameHtmlReport(g){const a=analyze([g]),tl=scoreTimeline(g);const score=`<div class="report-card full"><h2>Placar do jogo</h2><div style="display:flex;justify-content:center"><table style="width:260px;border-collapse:collapse;text-align:center;font-weight:900"><thead><tr><th></th><th>S</th><th>G</th><th>P</th></tr></thead><tbody><tr><td>AT</td><td>${tl.sets.athlete}</td><td>${tl.totalGames.athlete}</td><td>${a.won}</td></tr><tr><td>AD</td><td>${tl.sets.opponent}</td><td>${tl.totalGames.opponent}</td><td>${a.lost}</td></tr></tbody></table></div><p class="muted">${escapeHtml(gameEvolutionText(g))}</p></div>`;const body=`<section class="contentgrid">${score}<div class="report-card full"><h2>Comparativo técnico</h2>${indexComparisonHtml232(a,false)}</div>${barsHtml222('Desfechos do jogador',countBy(a.athleteEndings,'ending'),a.athleteEndings.length)}${barsHtml222('Erros do jogador por golpe',countBy(a.athleteErrors,'stroke'),a.athleteErrors.length)}<div class="report-card full"><h2>Leitura do treinador</h2><p>${escapeHtml(simplePhrase(a))}</p></div></section>`;return makeReportHtml222('Registro e leitura de jogo',`${g.date||''} · vs ${g.opponent||'Adversário'} · ${g.tournament||'Sem campeonato'} · ${levelLabel(g)}`,body);}
function pdocIndexRow(doc,x,y,w,r){const t=pdfTheme(),axis=x+w*.62,barMax=w*.18,bm=Math.min(100,Number(r.benchmark)||0),at=numericPctFromMetric(r.left),ad=numericPctFromMetric(r.mid);pdocText(doc,pdfMetricLabelCompact(r.label).slice(0,46),x,y,4.1,t.ink,false);pdocText(doc,r.leftDisplay,axis-barMax-3,y,4.1,t.ink,true,{align:'right'});doc.setFillColor(205,232,241);doc.rect(axis-barMax*(bm/100),y-2.8,barMax*(bm/100),3.6,'F');doc.setFillColor(83,207,255);doc.rect(axis-barMax*(at/100),y-2.2,barMax*(at/100),2.4,'F');doc.setFillColor(25,35,45);doc.rect(axis-.25,y-3.2,.5,4.3,'F');doc.setFillColor(242,220,185);doc.rect(axis,y-2.8,barMax*(bm/100),3.6,'F');doc.setFillColor(242,163,58);doc.rect(axis,y-2.2,barMax*(ad/100),2.4,'F');pdocText(doc,r.midDisplay,axis+barMax+3,y,4.1,t.ink,true,{align:'left'});}
function tableMetricsPdf222(doc,x,y,w,h,a){const t=pdfTheme();pdocCard(doc,x,y,w,h);pdocText(doc,'COMPARATIVO TÉCNICO',x+3,y+7,5.9,t.accent,true);pdocText(doc,'Índice',x+3,y+14,4.5,t.muted,true);pdocText(doc,'AT',x+w*.55,y+14,4.5,t.muted,true,{align:'center'});pdocText(doc,'AD',x+w*.76,y+14,4.5,t.muted,true,{align:'center'});const rows=comparisonRows(a).filter(r=>!r.section).slice(0,18);const rowH=(h-20)/Math.max(rows.length,1);rows.forEach((r,i)=>pdocIndexRow(doc,x+3,y+22+i*rowH,w-8,r));}
function exportGamePdf(){const JS=pdfCheckLib();if(!JS)return;const g=loadGames().find(x=>x.id===state.selectedGameId);if(!g)return;const a=analyze([g]),tl=scoreTimeline(g),doc=new JS({orientation:'landscape',unit:'mm',format:'a4'}),t=pdfTheme();doc.setFillColor(248,251,250);doc.rect(0,0,297,210,'F');pdocText(doc,'REGISTRO E LEITURA DE JOGO',14,14,7,t.accent,true);pdocText(doc,`Relatório - ${g.date||''} - vs ${g.opponent||'Adversário'} - ${levelLabel(g)}`,14,26,15,t.ink,true);pdocLine(doc,14,33,283,33,t.line,.45);drawScoreBySetPdf(doc,14,40,58,58,tl);onePresence222(doc,78,40,205,58,g);tableMetricsPdf222(doc,14,104,269,88,a);doc.save(`registro-leitura-jogo-${g.date||'sem-data'}-${pdfFileSafe(g.opponent||'adversario')}.pdf`);}
